    ...
    mounted: function () {
      ...
      this.$http.get(this.$store.state.CHINA_MAP_JSON)
        .then(function (chinaJson) {
          ...
          this.$http.get(this.$store.state.BASE_URL + '/citysYearEntEtabIndex')
            .then(function (response) {
                ...
                option = {
                  ...
                  title: [
                    {
                      text: '创业指数-'+timeLine[0]+'年重点城市成立企业数量(家)',
                      subtext: '锐信视界',
...
//                  max:10000,
                      min:   0,
                    max: (~~getCurrentYearMaxValue(timeLine[0]))-((~~getCurrentYearMaxValue(timeLine[0]))%5)+5 ,
//                    max: getCurrentYearMaxValue(timeLine[0]),
                      //将离散型的映射给分割了
                      splitNumber: 5,
                      // calculable: true,
                      inRange: {
